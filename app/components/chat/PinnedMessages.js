...
export default function PinnedMessages(props) {
    ...
    return (
        <div className="p-5 flex flex-col h-auto gap-4 md:gap-0 bg-inner-surface-light dark:bg-inner-surface-dark md:bg-light md:dark:bg-dark-accent
            ...
            <div className="flex md:flex-col gap-2 md:gap-0">
                {conversations.map((conversation) => {
                    const { avatarUrl, backgroundColor } = getConsistentAvatar(conversation.id)
                    return (
                        <div key={conversation.id}>
                            {/* Mobile */}
                            <div
                                className="w-[4rem] h-[5rem] xs:w-[5.5rem] xs:h-[7rem] mobileView md:hidden relative max-w-24 gap-2
                            ...
                            >
                                ...
                                <div className="flex-1">
                                    <div className="absolute bottom-1 right-2 xs:left-3 text-[.7rem] xs:text-[1rem] font-medium text-light truncate">
                                        {conversation.name?.split(' ')[0]}
                                    </div>
                                </div>
                                <button
                                    className="absolute -top-2 -right-2 p-1 bg-text-secondary-dark hover:bg-gray-200 
                                    rounded-full transition-opacity duration-200 border-2 border-light"
